...
public class Agent {
	 ...
	 */
	double E = 0.9;
	HashMap<String,yufan_StateActionPair> pair = new HashMap<String,yufan_StateActionPair>();
	HashMap<yufan_StateActionPair, Double> uMap = new HashMap<yufan_StateActionPair, Double>();
	HashMap<String,yufan_State> mdp = new HashMap<String,yufan_State>();
	
	HashMap<String,ArrayList<String>> s_saMap = new HashMap<String,ArrayList<String>>();
...
//		long start = System.currentTimeMillis();
		...
		yufan_State currentState = yufan_Util.learnState(this.currentBlock,
				this.board, mdp, s_saMap);

		if (R < E) {
			act = actRandomly();
			// learn this state and action
		} else {
			act = yufan_Util.actGreedy(currentState, uMap, s_saMap, pair);
			if (act == null) {
...
//		System.out.println("actLearnly use"
//				+ (System.currentTimeMillis() - start));
	...
	public void storeFile(String fileName) {
		...
		try {
			...
			for(String key : pair.keySet()){
				yufan_StateActionPair saP = pair.get(key);
				JSONObject jsonObject = writeToJSON(saP);
				json.add(jsonObject);
			}
	...
	public JSONObject writeToJSON(yufan_StateActionPair sap)
	{
		Double utility = this.uMap.get(sap);
		JSONObject jsonObject = new JSONObject();
		/*
		 * write state to json
		 * 1) current board -> json
		 * 2) current block -> json
		 */
		yufan_State state = sap.getS();
		
		//transit board to json
		...
		//transit block to json
		Block block = state.getCurrentBlock();
		String blockS ="";
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				blockS +=block.getPart(i, j);
			}	
		}
		
		/*
		 * write action to json
		 * 1) current action -> json
		 */
		int[] act = sap.getAct();
		String actS ="";
		...
		//put board , block to jsonOjbect
		jsonObject.put("board", boardS);
		jsonObject.put("block", blockS);
		//put act to jsonOjbect
		jsonObject.put("act",actS);
		jsonObject.put("utility", utility);
		return jsonObject;
	}
}