...
public class ValueIteration {
	...
	public Next goUp(State s, HashMap<State, Double> U) {
		...
		if (canReachTheNode(x - 1, y)) {
			// move up
			if (inRemainList(x - 1, y, s) && needToVisitThePoint(x - 1, y, s)) {
				...
				if (nextS == null) {
					System.out.println("s.size = " + s.remain.size());
					for (int i = 0; i < s.remain.size(); i++) {
						System.out.println(" x=" + s.remain.get(i).x + " y="
								+ s.remain.get(i).y);
					}
			...
			// stay
			if (inRemainList(x, y, s) && needToVisitThePoint(x, y, s)) {
				...
				if (currentCapacity == this.c
						&& inToVisit(new Point(s.x, s.y, mazeTable[s.x][s.y]))
						&& notInRemain(s.x, s.y, s) && mapIsZero(s)) {
					reward = 0;
				} else {
					reward = -1;
				}
	...
	public Next goLeft(State s, HashMap<State, Double> U) {
		...
		if (canReachTheNode(x, y - 1)) {
			// move up
			if (inRemainList(x, y - 1, s) && needToVisitThePoint(x, y - 1, s)) {
	...
	public boolean notInRemain(int x, int y, State s) {
		for (int i = 0; i < s.remain.size(); i++) {
	...
	public boolean needToVisitThePoint(int x, int y, State s) {
		double a = mazeTable[x][y];
		if (s.map.get(a) > 0)
			return true;
		return false;
	}

	public boolean mapIsZero(State s) {
	...
	public void updateSMap(int x, int y, State s) {
		double a = mazeTable[x][y];
		s.map.put(a, s.map.get(a) - 1);
	}
